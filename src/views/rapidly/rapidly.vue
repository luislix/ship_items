...
<script>
import { searchOrder,orderSendSmsToServer } from '@/api/index';
export default {
  name: "rapidly",
  data() {
    return {
      ...
      userData:'',
      userImg:'',
      Img:require("../../assets/image/logos.png")
    };
  },
  created(){
        ...
        if(this.userData !=''){
          ...
          searchOrder(type,this.message,phone).then(res=>{
            ...
            if(res.data == 'error'){
              ...
              let callPhone;
              if(this.userData.userType == 3){
                let index = res.data.indexOf('货主电话');
                callPhone = res.data.substr(index+5,11);
              } else {
                let index = res.data.indexOf('船主电话');
                callPhone = res.data.substr(index+5,11);
              }
              orderSendSmsToServer(callPhone,localStorage.getItem('token'))
            }
